...
namespace Semesterprojekt1PBA.Application.Features.School.Command
{
        ...
        : IRequestHandler<UpdateSchoolTitleCommand, Result<bool>>
    {
        public async Task<Result<bool>> Handle(UpdateSchoolTitleCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogError(ex, "Domain error occurred while updating the school title. ErrorCode: {ErrorCode}, UserMessage: {UserMessage}", ex.ErrorCode, ex.UserMessage);
            ...
            catch (Exception e)
            {
                ...
                return Result.Fail("Something went wrong while updating the school title. If the error happens again, contact support!");
            }
